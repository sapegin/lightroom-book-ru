...
Возможностей у плагина очень много, и подбробное описание плагина заняло бы полкниги, поэтому лучше посмотрите [руководство на сайте](http://www.photographers-toolbox.com/products/lrmogrify2.php?sec=quickguide).

### Другие способы

Если хочется чего-то необычного, то есть ещё два более экзотических способа добавления подписей и рамок:
...
Умная миниатюра — это уменьшенный рав-файл (2540 пикселей по длинной стороне), который позволяет использовать любые возможности Лайтрума для его проявки без доступа к исходному файлу. Каждая умная миниатюра занимает всего-лишь около мегабайта на диске независимо от объёма исходного рав-файла (например, для моего Canon 6D это примерно 25 МБ).

Генерируются умные миниатюры так: Library — Previews — Build Smart Previews. После этого под гистограммой появится надпись Original Photo:
...
Но на этом сложности не заканчиваются. Если фотографии находятся на сетевом диске, то обычный поиск операционной системы их не найдёт (***виндоус?***). В этом случае я использую [Toucan Search](https://itunes.apple.com/us/app/toucan-search/id427105394?mt=12) (для мака) — быстрый поиск по любым дискам, в том числе сетевым.
...
Коллекции удобны тем, что, при попадании туда, фотографии не копируются: в коллекцию помещается ссылка на фотографии. Можно спокойно удалять фотографии из коллекций — исходные файлы никуда  не денутся.

Чтобы создать коллекию, нажмите кнопку с плюсов в заголовке панели Collections и выберите Create Collection.

#### Добавление фотографий в коллекцию

Самый простой способ — просто перетащите снимок из библиотеки в коллекцию.

Но удобнее сделать коллекцию целевой: правый клик по названию коллекции в списке, а там Set as Target Collection. Справа от названия коллекции появится плюс:

![](images/target-collection.png)

Теперь можно добавить фотографии в эту коллекцию несколькими способами:

1. Add to Target Collection в контектном меню фотографии или клавиша B. Повторное нажатие удалит фотографию из коллекции.

2. Распылителем (как его использовать — см. в главе «Отбор»). Выберите Target Collection в списке Paint.

![](images/painter-target-collection.png)

### Умные коллекции

Кроме обычный коллекций в Лайтруме есть умные коллекции (Smart collections). Умные коллекции содержат не добавленные вручную фотографии, а отобранные по заданным в настройках коллекции критериям. Получается такая помесь коллекции и фильтра.
...
Чтобы создать умную коллекию, нажмите кнопку с плюсом в заголовке панели Collections и выберите Create Smart Collection. Теперь нужно добавить условия как показано на картинке:

![](images/unprocessed-collection.png)

Кроме необработанных фотографий, у меня есть коллекция отвергнутых (rejected) фотографий и коллекции фотографий с неполными метаданными:

![](images/health-collections.png)

Вы можете ***скачать*** настройки всех описанных в книге умных коллекций. Чтобы добавить такую коллекцию, нужно в контекстном меню панели коллекций кликнуть Import Smart Collection Settings и выбрать файл (с расширением .lrsmcol) нужной коллекции.
...
Не обязательно сразу создавать максимально полный список на все случи жизни. Достаточно продумать структуру и создать основные ключевые слова, а потом дополнять список по мере необходимости.
...
Первые два чекбокса стоит пояснить, а значение остальных понятно из их названия:

* Include on Export: нужно ли экспортировать ключевое слово. Полезно снимать для у ключевых слов — заголовков групп (например, «1. Where: Location»).
...
Чтобы добавить ключевые слова, нужно выделить одну или несколько фотографий, и поставить напротив нужных ключевых слов:
